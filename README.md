...
- **Model JSON is parsed fence-tolerantly (`parseModelJson` in `provider.ts`).** The prompt asks for a bare JSON object, but models — Haiku on the free tier especially — sometimes wrap it in a ` ```json … ``` ` markdown fence or add surrounding prose. A raw `JSON.parse` on that throws (`Unexpected token '\`'`), fails the job, and dead-letters the SQS message. `parseModelJson` strips a leading/trailing code fence and falls back to the outermost `{ … }` slice before parsing; genuinely non-JSON still throws (and is surfaced as a job failure). Caught D-147 full-loop smoke on dev.
